<template>
  ...
  <div class="p-4">
    ...
    <div class="relative">
        <select class="bg-gray-800 text-white py-2 px-4 rounded-md" v-model="selectedDate" @change="filterShowtimesByDate">
            ...
            <option v-for="date in uniqueShowDates" :key="date" :value="date">
              ...
              <div class="flex items-center space-x-2">
                    ...
                    <div class="relative w-full">
                        <select class="bg-gray-800 text-white py-2 px-4 rounded-md w-full" v-model="selectedMovie" @change="fetchCinemasByMovie(selectedMovie)">
                            <option value="">Chọn Phim</option>
                            <option v-for="showtime in filteredShowtimes" :key="showtime.showtimeId" :value="showtime.movieId">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <div class="relative w-full">
                        <p>{{ cinemas }}</p>
                        <select class="bg-gray-800 text-white py-2 px-4 rounded-md w-full" v-model="selectedCinema" @change="fetchMoviesByCinema(selectedCinema)">
          ...
          <div v-for="showtime in showtimes" :key="showtime.showtimeId"  class="grid grid-cols-1 md:grid-cols-3 gap-4 border-b border-gray my-4 py-4">
              ...
              <div>
                <div class="overflow-hidden">
                    <div class="text-xl font-bold mb-2 text-alight">
                    Cinestar
                    </div>
            ...
            <div >
                <div class="flex flex-wrap gap-2 p-2 border border-gray-400 rounded-md bg-gray-800 text-white text-sm shadow-sm">
                    <div v-for="time in showtime.showTime" :key="time"  class="inline-flex items-center justify-center px-3 py-1 border border-gray-500 rounded-md max-w-max">
                    {{formatTime(time)  }}
                    </div>
                </div>
            </div>
...
export default {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      const uniqueDates = [...new Set(this.showtimes.map(s => s.showDate))];
      this.uniqueShowDates = uniqueDates.map(date => this.formatDate(date));
    ...
    console.error("❌ Lỗi khi tải lịch chiếu:", error.response?.data || error.message);
  }
}
,
filterShowtimesByDate() {
  if (this.selectedDate) {
    ...
    this.filteredShowtimes = this.showtimes.filter(showtime => showtime.showDate === this.selectedDate);
...
fetchMoviesAndCinemasForSelectedDate() {
  ...
  this.movies = this.filteredShowtimes
    ...
    .filter((movie, index, self) => self.findIndex(m => m.id === movie.id) === index); // Loại bỏ trùng lặp

  // Lọc các rạp từ filteredShowtimes
  this.cinemas = this.filteredShowtimes
    .map(showtime => showtime.cinema)
    .filter((cinema, index, self) => self.findIndex(c => c.id === cinema.id) === index); // Loại bỏ trùng lặp

  // Hiển thị thông báo lỗi nếu không có phim hoặc rạp
  this.errorMessage = this.movies.length ? "" : "Không có phim nào vào ngày này!";
  this.errorMessage = this.cinemas.length ? "" : "Không có rạp chiếu vào ngày này!";
},
...
async fetchCinemasByMovie(movieId) {
  try {
    ...
    // Loại bỏ trùng lặp
    this.cinemas = [...new Map(this.cinemas.map(c => [c.id, c])).values()];

    this.errorMessage = this.cinemas.length ? "" : "Không có rạp nào chiếu phim này!";
  } catch (error) {
...
async fetchMoviesByCinema(cinemaId) {
  try {
    ...
    this.errorMessage = this.movies.length ? "" : "Không có phim nào trong rạp này!";
  } catch (error) {
